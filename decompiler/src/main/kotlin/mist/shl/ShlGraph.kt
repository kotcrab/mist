...
class ShlGraph(private val projectIO: ProjectIO, private val def: ShlFunctionDef, val logger: MistLogger) {
  val tag = logTag()
  lateinit var nodes: MutableList<Node<ShlInstr>>
    private set

  private val lifter = ShlLifter(projectIO, logger)
  ...
  private fun addEntryArgsAssigns(def: ShlFunctionDef, instrs: MutableList<ShlInstr>) {
    ...
    def.arguments.forEach { arg ->
      if (freeArgs.remove(arg.register) == false) {
        logger.panic(tag, "conflicting func. arg definitions (free arg list exhausted)")
      }
  ...
  fun removeNode(node: Node<ShlInstr>) {
    if (node.inEdges.size != 0) logger.panic(tag, "can't remove node which has input edges")
    logger.info(tag, "removing node ${nodes.indexOf(node)}")
