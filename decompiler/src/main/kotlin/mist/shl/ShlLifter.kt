...
class ShlLifter(private val projectIO: ProjectIO, private val logger: MistLogger) {
  val tag = logTag()
        ...
        when (it.opcode) {
          ...
          Nor -> {
            when {
              ...
              else -> logger.panic(tag, "nor can be only lifted to negation when one reg is set to reg zero")
            }
          }
          Sll, Sllv -> {
            shl.add(
          ...
          Jal -> {
            ...
            if (jalDef == null) {
              ...
              jalDef.arguments.forEach { arg ->
                if (freeArgs.remove(arg.register) == false) logger.panic(
                  tag,
          ...
          Jalr -> {
            if (it.op0AsReg() != GprReg.Ra) logger.panic(tag, "jalr doesn't use ra as return register")
...
//
          ...
          else -> logger.panic(tag, "not implemented ${it.opcode}")
        }
      }
    }
    return shl
  ...
  private fun mapMemOpcode(opcode: Opcode): ShlMemOpcode {
    return when (opcode) {
      ...
      else -> logger.panic(tag, "not an memory opcode: $opcode")
    }
  }
  ...
  private fun shlAuto(op: Operand?): ShlExpr {
    return when (op) {
      ...
      else -> logger.panic(tag, "can't convert operand to ShlExpr: $op")
    }
  }
}
