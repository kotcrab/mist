...
class DataFlowAnalysis(private val graph: ShlGraph, private val logger: MistLogger) {
  val tag = logTag()

  fun analyze() {
    logger.trace(tag, "analysis start")
    clearCtxs()
    reachRegisters()
    liveRegisters()
...
class LiveFlowCtx {
  ...
  fun dump(logger: MistLogger) {
    logger.trace(tag, "--- live flow ctx (where var is still alive) ---")
    varStates.forEach { varName, refs ->
      logger.trace(tag, "$varName = ${refs.joinToString()}")
    }
  }
...
class ReachFlowCtx {
  ...
  fun dump(logger: MistLogger) {
    logger.trace(tag, "--- reach flow ctx (where var is defined)---")
    varStates.forEach { varName, refs ->
      logger.trace(tag, "$varName = ${refs.joinToString()}")
    }
  }
...
class DataFlowCtx {
  ...
  fun dump(logger: MistLogger) {
    logger.trace(tag, "--- data flow ctx ---")
    varStates.forEach { varName, state ->
      logger.trace(tag, "$varName = $state")
    }
  }
